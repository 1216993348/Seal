...
function findCityByName(name){
  for (var i =0; i < arrCity.length;  i++) {
      if(arrCity[i].name == name){
        return arrCity[i].sub;
      }
  }
}
//根据省份、城市查找县区
function findCountyByName(parentName,name){
  for (var i =0; i < arrCity.length;  i++) {
    if(arrCity[i].name == parentName){
        if(arrCity[i].sub != undefined){
          for (var j =0; j < arrCity[i].sub.length;  j++) {
            if(arrCity[i].sub[j].name == name){
              return arrCity[i].sub[j].sub;
            }
          }
        }
    }
...
$(document).ready(function(){   
  ...
  for (var i =0; i < arrCity.length;  i++) {
    $("#province").append("<option>"+arrCity[i].name+"</option>");
  }
  $("#province").on("change",function(){
    /*if(app.editAddrInfo != undefined){
      app.editAddrInfo.city = "请选择";
    }*/
    if($(this).val() != "请选择" && $(this).val() != "其他"){
        $("#city").empty();
        $("#citySpan").show();
         var city = findCityByName($(this).val());
        if( city != undefined){
  ...
  $("#city").on("change",function(){
    if($(this).val() != "请选择" && $(this).val() != "其他"){
      $("#county").empty();
      var county =  findCountyByName($("#province").val(),$(this).val());
      if(county == undefined){
        $("#countySpan").hide();  
      }else{
        $("#countySpan").show();
        for (var i =0; i < county.length;  i++) {
          $("#county").append("<option>"+county[i].name+"</option>");  
        }
      }
    }else{
      $("#countySpan").hide();

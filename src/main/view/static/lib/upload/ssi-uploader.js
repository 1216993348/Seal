...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
         ...
         '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" index="'+index+'" data="'+id+'" class="ssi-button success ssi-hidden" >' +
         '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button info" index="'+index+'">' + this.language.clear +
         '</button>');
        ...
        $input.on('change', function () { //choose files
            ...
            thisS.uploadFiles('ssi-uploadBtn',"0");
            if(app.addMethodStatus == 4){
            	app.addMethodStatus = 2;
            }else{
            	app.addMethodStatus = 3;
            }
        ...
        $clearBtn.click(function () { //choose files completed and pending files
            thisS.clear();
            var index =$clearBtn.attr("index");
            if($("#preview-"+index).children().length == 2){
            	$("#preview-"+index).children("img:last").remove();
            }
            $("#oldImg-"+index).show();
            app.addMethodStatus =2;
        });
...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.uploadFiles = function (id,index) {// upload the pending files
        if (this.pending > 0) {
            ...
            //formData.append('url', $("#note-"+id).val());//append the first file to the form data
            formData.append('carId', id);//append the first file to the form data
            ...
            ajaxLoopRequest(formData, i,index);// make the request
        }

        //--------------start of ajax request-----------------------
        function ajaxLoopRequest(formData, ii,index) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                $("#oldImg-"+index).remove();
                $("#previewImg-"+index).attr("id","oldImg-"+index);
                $("[id='ssi-clearBtn'][index='"+index+"']").addClass('ssi-hidden');
                if (!thisS.options.preview) {
    ...
    var finishUpload = function (thisS) {//when every uplaod ends
        ...
        if (typeof thisS.options.onUpload === 'function') {
            try {
                //thisS.options.onUpload(resultData);//execute the on Upload callback
            } catch (err) {
    ...
    var locale = {
        ...
        gr: {
            ...
            clear: '取消',
            drag: 'Drag n Drop',
            sizeError: '$1 ~$2 文件大小不满足要求',// $1=file name ,$2=max file size ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 ~$ 文件类型不满足要求',// $1=file extension ie(exe files are not supported)
            someErrorsOccurred: 'Some errors occurred!'
        }
    };
